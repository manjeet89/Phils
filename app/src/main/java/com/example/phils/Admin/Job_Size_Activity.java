...
//        });
        ...
        profile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                dialog=new Dialog(Job_Size_Activity.this);
...
//        String [] items = withoutQuotes_line1.split("\\s*,\\s*");
        ...
        for (int i =0;i<items.length;i++) {
            ...
            if (items[i].equals("add-job-size")) { addjobsize = "add-job-size"; }
        }
        if(addjobsize.equals("add-job-size")){
    ...
    private void recycleClickLister() {
        listener = new JobSizeAdapterClass.RecycleViewClickListener() {
            ...
            public void onClick(View v, int position) {
                ...
                String updatejobsize = "";

                for (int i =0;i<items.length;i++) {

                    if (items[i].equals("update-job-size")) { updatejobsize = "update-job-size"; }
                }
                if(updatejobsize.equals("update-job-size")){
                    add.setVisibility(View.VISIBLE);
                }
                else
                {
                    add.setVisibility(View.GONE);
                    Toast.makeText(Job_Size_Activity.this, "No Access Available", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }
